...
[![dshfind](https://dshfind.com/api/badge/PerryLink/dsh-autotier?metric=downloads&lang=zh)](https://dshfind.com/zh/plugins/PerryLink/dsh-autotier?ref=badge)
...
| `@deepseek-ai/dsh` `0.1.7-alpha.2` | **必需**；已对同版 checkout（`typecheck`）与已发布包（`typecheck:ci`、214 项测试）双向验证 |
...
pnpm run typecheck:ci   # 对照已发布的 0.1.7-alpha.2 类型面（CI 实际执行）
pnpm test
pnpm run build

...
// default export, unresolved inject) exits non-zero with the reason on stderr.
...
try {
  ...
  // The plugin publishes the autotier service; its status pins that the Loader
  // applied the file's config (the mode comes from the row's config).
  const autotier = ctx.get('autotier')
  if (autotier === undefined) throw new Error('Loader composition: autotier service is missing')
  const status = autotier.status()
  process.stdout.write(`DSH_LOADER_RESULT ${JSON.stringify({
    mode: status.mode,
    strong: status.tiers.strong.model,
    cheap: status.tiers.cheap.model,
  })}`)
} catch (error) {
